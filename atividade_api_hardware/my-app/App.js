...
const takePicture = async () => {
  ...
  const fetchInspirationalQuote = async () => {
    ...
    try {
      ...
      // Função para salvar a citação API local
const salvarQuoteLocal = async () => {
  if (!currentQuote) return;

  try {
    const response = await fetch('http://192.168.0.:3000/usuarios', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        nome: currentQuote.content // usando "nome" como campo
      })
    });

    if (response.ok) {
      alert('Frase salva na sua API local!');
    } else {
      alert('Erro ao salvar frase');
    }
  } catch (error) {
    console.error('Erro ao salvar na API local:', error);
  }
};
  ...
  <View style={{ flex: 1 }}>
    {hasCameraPermission ? (
      ...
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: currentTheme.background }}>
        ...
        >
          <Text style={{ color: 'white', fontWeight: '600' }}>Solicitar Permissão</Text>
        </TouchableOpacity>
      </View>
    )}
  </View>
